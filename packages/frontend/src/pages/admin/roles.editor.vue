	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.required2fa, 'required2fa'])">
				...
				<template #suffix>
					<span v-if="role.policies.required2fa.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.required2fa.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.required2fa)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.required2fa.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.required2fa.value" :disabled="role.policies.required2fa.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.required2fa.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canUseAdvancesSearch, 'canUseAdvancesSearch'])">
				<template #label>{{ i18n.ts._role._options.canUseAdvancesSearch }}</template>
				<template #suffix>
					<span v-if="role.policies.canUseAdvancesSearch.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canUseAdvancesSearch.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canUseAdvancesSearch)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canUseAdvancesSearch.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canUseAdvancesSearch.value" :disabled="role.policies.canUseAdvancesSearch.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canUseAdvancesSearch.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.accessTokenMax, 'accessTokenLimit'])">
				...
				<template #suffix>
					<span v-if="role.policies.accessTokenLimit.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.accessTokenLimit.value }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.accessTokenLimit)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.accessTokenLimit.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkInput v-model="role.policies.accessTokenLimit.value" :disabled="role.policies.accessTokenLimit.useDefault" type="number" :readonly="readonly">
					</MkInput>
					<MkRange v-model="role.policies.accessTokenLimit.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
